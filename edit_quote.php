...
if(1)//只有管理员可以看到编辑选项
{
	...
	if(isset($_GET['id'])&&is_numeric($_GET['id'])&&($_GET['id']>0))
	{
		...
		echo '<form class="left" action="edit_quote.php" method="post" >';
		if($r=mysql_query($query,$dbc)){
			//print"查找到数据库";
		$row=mysql_fetch_array($r);
		echo  
		'<div style="padding:20px 20px 0px 20px; width:95%; height:auto;"><p style=" text-align:left;"><label>内容:<textarea id="quotes" name="quote" >' .$row['quote'].'</textarea></label></p></br><p style=" text-align:left;"><label>详情:<textarea id="detail" name="detail" >' .$row['detail'].'</textarea></label></p></br><p style=" text-align:left;"><label>来源：<input id="source" class="afterClear" type="text" name="source" value="' .$row['source'] . '"></label></p></div>
		';
		//print"是不是已经定义";
  ...
  elseif( isset($_POST['id'])&&is_numeric($_POST['id'])&&($_POST['id']>0))
  {
	...
	if(!$problem)
			 {
				 $query="UPDATE quotes SET quote='$quote',detail='$detail',source='$source' WHERE quote_id={$_POST['id']}";
				 if($r=mysql_query($query,$dbc))
				 {
					 print'<script>alert("数据已更新");
